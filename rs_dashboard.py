...
def _analyse(trade_history, closes, cfg):
    ...
    # 🛡️ 月度格只用換倉日；平倉日（含止損）對應到下一個換倉日
    import bisect
    _grid = list(months)
    _gi = {d: i for i, d in enumerate(_grid)}
    _held = {d: [] for d in _grid}
    _hold_names = {d: set() for d in _grid}
    for r in rows:
        i0 = _gi.get(r.get('date'))
        if i0 is None: continue
        _cd = r.get('close_date')
        i1 = bisect.bisect_right(_grid, _cd) if _cd else i0 + 1
        i1 = min(max(i1, i0 + 1), len(_grid))
        n = i1 - i0
        _mr = (1 + r['ret']) ** (1 / n) - 1
        for k in range(i0, i1):
            _held[_grid[k]].append(_mr)
            _hold_names[_grid[k]].add(r['tk'])
    months = [d for d in _grid if _held[d]]
